...
| 差 1 分 | 55.6%（5 vs 4）到 52.9%（9 vs 8） | **68%** 仅取决于分差 |
| 差 2 分 | 60%（6 vs 4）到 55%（10 vs 8） | **81%** |
| 差 3 分 | 63.6%（7 vs 4）到 57.1%（10 vs 7） | **90%** |
| K 值 | — | `K = 0.7324`（固定，差3分≈九一开） |

原版比例模型 `A/(A+B)` 的同一分差在不同绝对值上胜率不同。Logistic 模型只关心分差，不关心绝对值，评分尺度一致。
